...
typedef RouterListener = void Function();

/// Public router contract used by Unrouter integrations.
abstract interface class Unrouter<V> {
  ...
  roux.Router<RouteRecord<V>> get matcher;

  /// Adds a listener that is called after an accepted location change.
...
Unrouter<V> createRouter<V>({
  required Iterable<RouteNode<V>> routes,
...
extension<V> on RouteNode<V> {
  Map<String, String> makeAliasRoutes() {
    final routes = <String, String>{};
    void collect(RouteNode<V> route, String parentPath) {
  ...
  Map<String, RouteRecord<V>> makeRouteRecords([Iterable<Guard>? global]) {
    final routes = <String, RouteRecord<V>>{};

    void collect(
      RouteNode<V> route,
      String parent,
      Iterable<ViewBuilder<V>>? parentViews,
      Iterable<Guard>? parentGuards,
      Map<String, Object?>? parentMeta,
    ) {
      final path = normalizePath([parent, route.path]);
      final views = <ViewBuilder<V>>[...?parentViews, route.view];
...
final class _RouterImpl<V> implements Unrouter<V> {
  _RouterImpl({
  ...
  @override
  final roux.Router<RouteRecord<V>> matcher;

  final int maxRedirectDepth;
  final void Function(Object error, StackTrace stackTrace) errorReporter;

  final Set<RouterListener> _listeners = <RouterListener>{};
  ...
  _NavigationTarget<V> _resolveNavigationTarget(
    String pathOrName, {
    Map<String, String>? params,
    URLSearchParams? query,
    Object? state,
  ...
  _NavigationTarget<V> _resolveByPath({
  ...
  Future<_GuardEvaluation<V>> _runGuards({
    required HistoryAction action,
    required HistoryLocation from,
    required _NavigationTarget<V> target,
  }) async {
    var current = target;
    var redirects = 0;
    while (true) {
...
final class _GuardEvaluation<V> {
  ...
  final _NavigationTarget<V>? target;
}

final class _NavigationTarget<V> {
